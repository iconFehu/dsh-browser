...
describe('TabAffinityController', () => {
  ...
  it('retains multiple controlled tabs while switching the current target', () => {
    const affinity = new TabAffinityController()
    affinity.bindNewSession('session', tab(1))
    affinity.addSessionTab('session', tab(2))
    expect(affinity.getSessionTabs('session').map((item) => item.tabId)).toEqual([1, 2])
    expect(affinity.resolveTarget('session')).toMatchObject({ kind: 'target', tab: { tabId: 2 } })
    expect(affinity.selectSessionTab('session', tab(1))).toBe(true)
    expect(affinity.getSessionTabs('session').map((item) => item.tabId)).toEqual([1, 2])
    const restored = new TabAffinityController()
    restored.restoreSessionTabs(affinity.sessionMap())
    restored.restoreSessionMembers(affinity.sessionMembersMap())
    expect(restored.getSessionTabs('session').map((item) => item.tabId)).toEqual([1, 2])
