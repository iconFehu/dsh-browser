...
describe('tool-driven session checkpoints', () => {
  it('updates only after a successful navigation candidate', async () => {
    ...
    const call = async (id: string, name: string): Promise<void> => {
      const [capability, ...method] = name.split('.')
      socket.receive({
        t: 'capability.call',
        id,
        capability,
    ...
    socket.receive({
      t: 'capability.call',
      id: 'committed',
      capability: 'management', method: 'tabs.navigate',
    ...
    socket.receive({ t: 'capability.cancel', id: 'committed' })
    finishCommitted()
    await vi.waitFor(() => {
      expect(socket.sent).toContainEqual(expect.objectContaining({ t: 'capability.result', id: 'committed', ok: true }))
    })
