...
$chromeArchive = @(Get-ChildItem -LiteralPath $bundle -Filter 'dsh-browser-chrome-*.zip' -File)
if ($chromeArchive.Count -ne 1) { throw 'Expected exactly one dsh-browser-chrome-*.zip in the bundle' }
$chromeName = $chromeArchive[0].Name
$version = $chromeArchive[0].BaseName.Substring('dsh-browser-chrome-'.Length)
...
try {
  ...
  $oldManifest.version = '0.0.1'
  $oldManifest | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $manifest -Encoding UTF8
  & $installer -BundleDirectory $bundle -DshHome $testHome -NoOpen
  Assert ((Get-Content -LiteralPath $manifest -Raw | ConvertFrom-Json).version -eq $version) 'Version upgrade failed'
  $bad = Join-Path $testRoot 'bad-bundle'
  New-Item -ItemType Directory -Path $bad -Force | Out-Null
  Copy-Item -LiteralPath (Join-Path $bundle $chromeName) -Destination $bad
  Set-Content -LiteralPath (Join-Path $bad 'SHA256SUMS.txt') -Value ('0' * 64 + "  $chromeName")
