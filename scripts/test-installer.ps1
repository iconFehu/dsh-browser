...
try {
  ...
  & $installer -BundleDirectory $bundle -DshHome $testHome -NoOpen
  Assert ((Get-Content -LiteralPath $manifest -Raw | ConvertFrom-Json).version -eq '0.1.5') 'Version upgrade failed'
  $bad = Join-Path $testRoot 'bad-bundle'
  New-Item -ItemType Directory -Path $bad -Force | Out-Null
  Copy-Item -LiteralPath (Join-Path $bundle 'dsh-browser-chrome-0.1.5.zip') -Destination $bad
  Set-Content -LiteralPath (Join-Path $bad 'SHA256SUMS.txt') -Value ('0' * 64 + '  dsh-browser-chrome-0.1.5.zip')
