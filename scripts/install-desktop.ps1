# Prebuilt DSH Desktop installer for the Chrome extension.
# Windows PowerShell 5.1+; no Git, Node, pnpm or administrator privileges required.
# Run it from an extracted dsh-browser-windows.zip release bundle, or pass -Version to download that release.
[CmdletBinding()]
param(
  [string]$Version,
...
if (-not $Version) {
  $bundled = @(Get-ChildItem -LiteralPath $BundleDirectory -Filter 'dsh-browser-chrome-*.zip' -File -ErrorAction SilentlyContinue)
  if ($bundled.Count -ne 1) { throw '请在解压后的 Windows 安装包中运行，或用 -Version 指定版本 / Run from an extracted release bundle or pass -Version.' }
  $Version = $bundled[0].BaseName.Substring('dsh-browser-chrome-'.Length)
}
