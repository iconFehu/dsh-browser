...
.DESCRIPTION
标准 web 源码安装；Desktop 用户请使用预构建安装器并开启浏览器访问。
Source installation for standard web. Desktop users should use the prebuilt installer and enable browser access.
...
$ArchiveUrl = "https://github.com/$Repository/archive/refs/tags/$RemoteRef.zip"
...
function Install-RemoteWorkspace {
  ...
  if ((Test-Path -LiteralPath $ManagedRoot -PathType Container) -and
    ...
    Stop-Install `
      "$ManagedRoot 已存在且不是脚本托管的安装目录；为避免覆盖，请移动该目录或在其中运行 .\scripts\install.ps1 -Runtime Web。" `
      "$ManagedRoot already exists and is not managed by this installer; move it or run .\scripts\install.ps1 -Runtime Web inside it to avoid overwriting it."
  }
