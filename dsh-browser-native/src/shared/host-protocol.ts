...
export type HostRequest =
  ...
  | { type: 'tool.result'; id: string; ok: false; error: { code: string; message: string } }
...
export function isHostRequest(value: unknown): value is HostRequest {
  ...
  if (frame.type === 'tool.result') return typeof frame.id === 'string' && (frame.ok === true || (frame.ok === false && typeof frame.error === 'object' && frame.error !== null))
