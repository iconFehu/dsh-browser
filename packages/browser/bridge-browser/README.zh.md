...
提示词中的多个 `[[dsh-browser-tab:<ref>]]` 标记会在模型步骤前移除，并将对应标签页全部绑定到会话。最后选择的标签页是当前目标；可用 `management.tabs.activate` 在会话的标签页之间切换。Web 客户端的 `@tab` 选择框从 `/ext/browser-tabs` 读取候选 ref；这个接口只响应来自宿主自身 Web UI 的回环同源请求，其他请求一律返回 403。
