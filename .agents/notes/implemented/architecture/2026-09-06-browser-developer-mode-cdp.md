...
- **Observation only.** Clicks, typing, scrolling and navigation keep the high-level content-script pipeline untouched — Codex's division of labor. CDP attaches to the controlled tab purely to observe: `cdp.dom` (deep per-frame text incl. open shadow DOM and uninjectable iframes, no inventory), `cdp.diagnostics` (console/Log/network failures), `cdp.network` (request list lines include `requestId=<id>` so the model can drive `Network.getResponseBody`; `includeBodies` fetches capped bodies with the same requestId in the body header and a sensitive-data warning), `cdp.performance` (counter deltas), and `cdp.captureScreenshot`/`cdp.exportPdf` (PNG/PDF via `Page.captureScreenshot`/`Page.printToPDF`, exported through a **save dialog** to a local file).
...
- **Model surface.** The model reaches these observations through the `cdp` capability tool: `cdp.call` with an allowlisted method (`Accessibility.getFullAXTree`, `DOM.getDocument`/`getOuterHTML`, `Network.enable`/`disable`/`getResponseBody`, `Performance.*`, `Page.captureScreenshot`, `Page.printToPDF`) is translated by `resolveCdpCall` to the matching observation, and `cdp.events` reads diagnostics. `Network.getResponseBody` is an intentional pass-through (params `{ requestId }`) that reuses `manager.fetchResponseBody` with the same `NETWORK_BODY_CHARS` (~8000) cap and sensitive-data warning as `cdp.network` `includeBodies`. Any other method fails closed.
