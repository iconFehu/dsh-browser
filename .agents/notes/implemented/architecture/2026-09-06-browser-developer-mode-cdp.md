...
- **Observation only.** Clicks, typing, scrolling and navigation keep the high-level content-script pipeline untouched — Codex's division of labor. CDP attaches to the controlled tab purely to observe: `browser_dom` (deep per-frame text incl. open shadow DOM and uninjectable iframes, no inventory), `browser_diagnostics` (console/Log/network failures), `browser_network` (request list; `includeBodies` fetches capped `Network.getResponseBody` text with a sensitive-data warning), `browser_performance` (counter deltas), and `browser_screenshot`/`browser_export_pdf` (PNG/PDF via `Page.captureScreenshot`/`Page.printToPDF`, exported through a **save dialog** to a local file).
