...
- **Model surface.** The model reaches these observations through the `cdp` capability tool: `cdp.call` with an allowlisted method (`Accessibility.getFullAXTree`, `DOM.getDocument`/`getOuterHTML`, `Network.*`, `Performance.*`, `Page.captureScreenshot`, `Page.printToPDF`) is translated by `resolveCdpCall` to the matching observation, and `cdp.events` reads diagnostics. Any other method fails closed.
